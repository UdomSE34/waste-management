...
const SalaryDashboard = () => {
  ...
  const toggleSalaryStatus = async (salary) => {
    ...
    try {
      ...
      // Option 1: Update state immediately
      setUsers((prev) =>
        prev.map((u) =>
  ...
  const handleExportPDF = () => {
    window.open("/api/salary/salaries/export_pdf/", "_blank");
  };
  ...
  //   unpaid: { label: "Unpaid", class: "status-upaid" },
  ...
  //   };
  //   return (
  //     <span className={`status-paid ${config.class}`}>{config.label}</span>
  //   );
  // }
